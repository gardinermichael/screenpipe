...
function getApiKey(): string {
  ...
  if (!key) {
    ...
    console.error("Alternative: configure the official Linear MCP server (mcp.linear.app) and skip this script.");
    process.exit(1);
  }
  return key;
}
...
async function listProjects(args: string[]) {
  ...
  // Use GraphQL variables — never interpolate user input into the query string.
  if (team) {
    const data = (await gql(
      `query($teamKey: String!) {
        projects(first: 50, filter: { accessibleTeams: { some: { key: { eq: $teamKey } } } }, orderBy: updatedAt) {
          nodes {
            id name state
            teams { nodes { key } }
            createdAt updatedAt
...
async function listIssues(args: string[]) {
  ...
  const limitStr = getFlag(args, "--limit") || "25";
  const limit = Math.min(Math.max(parseInt(limitStr, 10) || 25, 1), 100);

  // Build filter object as GraphQL variables — no string interpolation.
  const filter: Record<string, unknown> = {};
  if (team) filter.team = { key: { eq: team } };
  if (status) filter.state = { name: { eq: status } };
  if (assignee) filter.assignee = { name: { containsIgnoreCase: assignee } };
  ...
  const data = (await gql(
    ...
    { filter: Object.keys(filter).length ? filter : null, first: limit }
